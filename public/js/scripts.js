...
socket.on('user_connected', (username) => {
  drawNewChat(`${username}님 입장하셨습니다.`);
});
socket.on('new_chat', (data) => {
  const { chat, username } = data;
  drawNewChat(`${username}: ${chat}`);
});
socket.on('disconnect_user', (username) => drawNewChat(`${username}님 퇴장하셨습니다.`));
...
const drawHelloStranger = (username) =>
  (helloStrangerElement.innerText = `네스트챗에 오신 걸 환영합니다. ${username}님`);
